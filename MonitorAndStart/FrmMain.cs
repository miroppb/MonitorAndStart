using MonitorAndStart.Properties;
using System;
...
namespace MonitorAndStart
{
	public partial class FrmMain : Form
	{
		...
		public FrmMain()
		{
			...
			try
			{
				string[] a = Settings.Default.Items.Split(';');
			...
			TxtTimeStart.Text = Settings.Default.TimeIgnoreStart.ToShortTimeString();
			TxtTimeEnd.Text = Settings.Default.TimeIgnoreEnd.ToShortTimeString();
			if (Settings.Default.CheckOnStart)
		...
		{
			DateTime start = DateTime.Parse(DateTime.Now.ToShortDateString() + " " + Settings.Default.TimeIgnoreStart.ToShortTimeString());
			DateTime end = DateTime.Parse(DateTime.Now.ToShortDateString() + " " + Settings.Default.TimeIgnoreEnd.ToShortTimeString());
			if (DateTime.Now.TimeOfDay < start.TimeOfDay || DateTime.Now.TimeOfDay > end.TimeOfDay) //run only if before specified time or after specified time
			{
					...
					else if (file.StartsWith("Script:"))
					{
						...
						if (scripts[filename].ToString("MM/dd/yyyy htt") == DateTime.Now.ToString("MM/dd/yyyy htt") || scripts[filename] <= DateTime.Now)
						{
							try
							{
								...
								if (p.ExitCode != 0)
								{
									scripts[filename] = DateTime.Now.AddHours(1);
									LstItems.Items[LstItems.Items.IndexOf(file)] = $"Script:{filename},{par},{1},{line.Split(',')[3]},{DateTime.Now}";
								}
								else
								{
		...
		private void FrmMain_Load(object sender, EventArgs e)
		{
			...
			ChkAdmin.Checked = Settings.Default.Admin;
		...
		private void ChkAdmin_CheckedChanged(object sender, EventArgs e)
		{
			if (!starting)
			{
				Settings.Default.Admin = ChkAdmin.Checked;
				Settings.Default.Save();
		...
		private void ChkCheckOnStart_CheckedChanged(object sender, EventArgs e)
		{
			if (!starting)
			{
				Settings.Default.CheckOnStart = ChkCheckOnStart.Checked;
				Settings.Default.Save();
		...
		private void TxtTimeStart_MouseDoubleClick(object sender, MouseEventArgs e)
		{
			FrmTime f = new FrmTime();
			//if the cell wasn't empty, fill the time
			try { f.Dtp.Value = DateTime.Parse(DateTime.Now.ToShortDateString() + " " + TxtTimeStart.Text.ToString()); }
			catch { f.Dtp.Value = DateTime.Now; }
			if (f.ShowDialog() == DialogResult.OK)
			{
				//after window is closed, if OK, get the time, and insert into field
				TxtTimeStart.Text = f.Dtp.Value.ToShortTimeString();
				//save to settings
				Settings.Default.TimeIgnoreStart = f.Dtp.Value;
				Settings.Default.Save();
			}
		}

		private void TxtTimeEnd_MouseDoubleClick(object sender, MouseEventArgs e)
		{
			FrmTime f = new FrmTime();
			//if the cell wasn't empty, fill the time
			try { f.Dtp.Value = DateTime.Parse(DateTime.Now.ToShortDateString() + " " + TxtTimeEnd.Text.ToString()); }
			catch { f.Dtp.Value = DateTime.Now; }
			if (f.ShowDialog() == DialogResult.OK)
			{
				//after window is closed, if OK, get the time, and insert into field
				TxtTimeEnd.Text = f.Dtp.Value.ToShortTimeString();
				//save to settings
				Settings.Default.TimeIgnoreEnd = f.Dtp.Value;
				Settings.Default.Save();
			}
		}
	}
